...
os.system("cat "+domain+"/urls_slice.txt | nuclei -t /root/nuclei-templates/cves/2022/CVE-2022-28923.yaml -t /root/nuclei-templates/vulnerabilities/generic/open-redirect.yaml > "+domain+"/openredirect.txt")

os.system("cat "+domain+"/urls_slice.txt | nuclei -tags git,crlf > "+domain+"/git-crlf.txt")

os.system("cat "+domain+"/urls_slice.txt | nuclei -t  multiples-swagger-xss-indentify.yaml > "+domain+"/swagger-xss.txt")

os.system("cat "+domain+"/urls_slice.txt | httpx -title | grep 'Index of' > "+domain+"/indexof.txt")

os.system("cat "+domain+"/urls_slice.txt | nuclei -tags traversal > "+domain+"/traversal.txt")
